...
Antworten = []
df = pd.DataFrame(Antworten, columns = ["Frage1",  "Frage2"])
...
class TypQuiz(FlaskForm):
    ...
    q4 = RadioField(
        "4. Stell dir vor, du kaufst dir eine Aktie und du siehst, dass die Aktie mehr Wert gewinnt. Wie verhältst du dich:",
...
def form():
    ...
    if form.validate_on_submit(): 
        Antworten.append([form.q1.data, form.q2.data])
        # 1. Neue Pandas erstellen oder öffnen existierende
        # 2. Speicherung neuer Daten (hinzufügen)
        # 3. Speichern / Datei schließen
        return '<center><h1>Du bist Typ Baboon</h1></center> <break> <center><h2> Du hast bei Frage 1 {}. geantwortet <break> und bei Frage 2 {}. geantwortet <break> {} </h2></center>'.format(form.q1.data, form.q2.data) #Insert jump to fitting Typ-Page here
